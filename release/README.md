# `brond`'s Reproducible Build System

This package contains the build script that the `brond` project uses in order to
...
This will then create a directory of the form `brond-<TAG>` containing archives
...
Now that the directory `brond-<TAG>` is created, the manifest file needs to be
...
When following the GitHub documentation, include every file in the `brond-<TAG>`
directory.
...
5. Ensure `go` is installed, matching the same version as noted in the release
   notes. 
6. Obtain a copy of `brond`'s source code with `git clone
...
8. Extract the archive found in the `brond-<TAG>` directory created by the
   release script and recompute the `SHA256` hash of the release binaries (brond
   and bronctl) with `shasum -a 256 <filename>`. These should match __exactly__
