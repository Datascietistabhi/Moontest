...
// nodeConfig contains all the args, and data required to launch a brond process
...
func newConfig(prefix, certFile, keyFile string, extra []string,
	...
	var brondPath string
	if customExePath != "" {
		brondPath = customExePath
	} else {
		var err error
		brondPath, err = btcdExecutablePath()
		if err != nil {
			brondPath = "brond"
		}
	}
...
// arguments returns an array of arguments that be used to launch the brond
// process.
...
// command returns the exec.Cmd which will be used to start the brond process.
...
// to the brond process that is launched via Start().
...
// as the base for the log and data directories for brond.
...
// start creates a new brond process, and writes its pid in a file reserved for
...
// stop interrupts the running brond process process, and waits until it exits
...
// shutdown terminates the running brond process, and cleans up all
