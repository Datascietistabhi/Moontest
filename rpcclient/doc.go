...
Bitcoin RPC server that uses a brond/brocoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with brond (https://github.com/bronsuite/brond),
bronwallet (https://github.com/bronsuite/bronwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, brond and
bronwallet provide a websocket interface that is more efficient than the standard
...
brond or bronwallet by default.  However, configuration options are provided to
...
bronwallet only uses a single connection that remains open and allows
...
bronwallet passthrough).  Therefore if you call one of these commands against
...
functionality into a separate process named bronwallet.  This means if you are
...
need chain-related RPCs.  In contrast, bronwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - brondwebsockets
   ...
   block count
 - bronwalletwebsockets
   Connects to a bronwallet RPC server using TLS-secured websockets, registers
