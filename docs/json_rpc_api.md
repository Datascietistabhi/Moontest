...
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  ...
  [bronwallet](https://github.com/bronsuite/bronwallet).
...
such as [bronwallet](https://github.com/bronsuite/bronwallet) for inter-process
...
during the development of bronwallet.
...
brond is `wss://your_ip_or_domain:8221/ws`.
...
brond comes with a separate utility named `bronctl` which can be used to issue
...
|Notes|NOTE: Since brond does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in bronwallet for a version which includes that information.|
